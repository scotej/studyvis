...
`@trystero-p2p/core` declares a peer closed **5 s** after its `RTCPeerConnection` reports `disconnected` (`disconnectedCloseDelayMs` in its `peer.mjs`), and that detaches **every** room for that friend — `SharedPeerManager` keeps one `RTCPeerConnection` per (appId, peerId), so the session, presence, and inbox rooms all bind to the same transport. Five seconds is shorter than a Wi-Fi roam and far shorter than the multi-second renderer stalls a machine running the on-device model produces.

It does **not** close that connection, and the resulting half-teardown is what #264's reporter actually experienced. `SharedPeerManager.clear(…, {destroyPeer: false})` empties `shared.bindings` *before* firing each binding's close handler, so the `detachBinding` reached through the room's `proxy.destroy()` early-returns and `pruneRoomOwnership` — the only thing that removes our senders — never runs. The connection is abandoned open: once ICE recovers, **camera and screen keep transmitting to a peer the UI has already reported gone**, while the session room, its chat, and every other room are unbound. Reconnection is wedged too, on the **far** side: our own rooms clear `state.connectedPeer` (`strategy.mjs`, via the `onPeerLeave` that `exitPeer` calls), but the friend's side never closed anything, so their `state.connectedPeer` still points at this connection, `getConnectedPeerHealth` reads `"live"` on it precisely because our data channel never closed, and `createSignalHandler` returns early on exactly that condition — dropping every offer, answer and candidate we send, forever. (This also explains the field split in the issue's logs: a drop where the connection genuinely died read `"stale"` and re-formed in 14 s; the drop where it recovered never came back.) `wireSessionRoom`'s departure path therefore closes an abandoned transport itself — data channels first, so the friend learns from a stream reset instead of waiting out ICE consent — which both stops the transmission and flips that health to `"stale"` so re-establishment can proceed.
...
keeps a name-only "Reconnecting…" tile for that peer for 30 s — long enough
for trystero to re-establish, which it can do only because that same path
closes the abandoned connection first (see §4).
