...
function capturedDisplay(surface?: string): {
  stream: MediaStream
  ...
  return {
    stream: {
      getTracks: () => [videoTrack, auxiliaryTrack],
      getVideoTracks: () => [videoTrack],
    } as unknown as MediaStream,
    stops: [videoStop, auxiliaryStop],
  }
}
...
describe('requestScreenShareStream', () => {
  test('prefers and accepts a whole monitor on Windows', async () => {
    const capture = capturedDisplay('monitor')
    const getDisplayMedia = installDisplayMedia(
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Edg/130.0.0.0',
      capture.stream
    )

    await expect(requestScreenShareStream()).resolves.toBe(capture.stream)
    ...
    for (const stop of capture.stops) expect(stop).not.toHaveBeenCalled()
  })

  test.each(['window', 'browser'])(
    'stops and rejects a Windows %s selection before publication',
    async (surface) => {
      const capture = capturedDisplay(surface)
      installDisplayMedia(
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Edg/130.0.0.0',
        capture.stream
      )
      ...
      await expect(attempt).rejects.toMatchObject({ selectedSurface: surface })
  ...
  test('fails closed and releases an unverifiable Windows selection', async () => {
    ...
    await expect(requestScreenShareStream()).rejects.toMatchObject({
      name: 'ScreenShareMonitorRequiredError',
      selectedSurface: undefined,
    })
    for (const stop of capture.stops) expect(stop).toHaveBeenCalledTimes(1)
  })
  ...
  ])('leaves the existing picker behavior unchanged on %s', async (_os, ua) => {
    const capture = capturedDisplay('window')
    const getDisplayMedia = installDisplayMedia(ua, capture.stream)
  ...
  test('preserves getDisplayMedia denial errors', async () => {
    ...
    await expect(requestScreenShareStream()).rejects.toBe(denied)
  })
})
