...
describe('session image wire', () => {
  ...
  test('rejects truncated and inconsistent image containers', () => {
    expect(
      inspectImageBytes(VALID_PNG_BYTES.slice(0, 20), 'image/png')
    ).toBeNull()
    expect(
      inspectImageBytes(VALID_JPEG_BYTES.slice(0, 6), 'image/jpeg')
    ).toBeNull()
    expect(
      inspectImageBytes(VALID_GIF_BYTES.slice(0, 12), 'image/gif')
    ).toBeNull()
    expect(
      inspectImageBytes(VALID_WEBP_BYTES.slice(0, 16), 'image/webp')
    ).toBeNull()

    const badPngLength = VALID_PNG_BYTES.slice()
    badPngLength[11] = 30
    expect(inspectImageBytes(badPngLength, 'image/png')).toBeNull()
    const badWebpLength = VALID_WEBP_BYTES.slice()
    badWebpLength[4] = 0xff
    expect(inspectImageBytes(badWebpLength, 'image/webp')).toBeNull()
  })
  ...
  test('rejects tampered bytes, metadata, sender, and session', async () => {
    ...
    expect(
      verifyIncomingImage(
        payload.bytes,
        { ...payload.metadata, sig: '00'.repeat(64) },
        signer.edHex,
        TOPIC
      )
    ).toBeNull()
  ...
  test('drops an image whose decode finishes after the session stops', async () => {
    const { signer, payload } = await fixture()
    const verified = verifyIncomingImage(
      payload.bytes,
      payload.metadata,
    ...
    const deferredDecode = new Promise<{ width: number; height: number }>(
      (resolve) => {
        finishDecode = resolve
      }
    )
    const readDimensions = vi.fn(() => deferredDecode)
    const append = vi.fn()
    let stopped = false
    const acceptance = appendVerifiedIncomingImage(verified!, {
      isStopped: () => stopped,
      append,
      readDimensions,
    })
...
describe('session image save', () => {
  test('passes exact bytes to the native image-save command', async () => {
    ...
    expect(saveFile).toHaveBeenCalledWith({
      filename: 'image.png',
      mimeType: 'image/png',
      bytes: Array.from(VALID_PNG_BYTES),
    })
  })
