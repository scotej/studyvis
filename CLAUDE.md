...
- **The Linux runtime is a product-owned supply-chain surface.** Official distro WebKitGTK builds in the maintained path compile the `ENABLE_WEB_RTC` peer-connection binding out while retaining `navigator.mediaDevices`; do not “fix” Linux by linking the AppImage back to the host copy. Keep `scripts/linux-webkit-runtime.env`, `scripts/build-linux-webkit-runtime.sh`, `scripts/patches/webkitgtk-2.52.5-appimage-sandbox.patch`, its AppImage-relative WebKit-process/injected-bundle/sandbox-helper layout, the vendored wry constructor delta, `BUILD-MANIFEST.txt`, the 59-file readable/hash-addressed WebKit license inventory, the generated librice transitive notice pair, deterministic corresponding-source archive + checksum, and AppImage checks coherent. Keep the separate source-built AppImage type-2 runtime revision 2 tuple and its system-source evidence coherent too: exact type2/musl/zlib/decompression-only-zstd/libfuse/squashfuse/Meson sources and licenses, Noble compiler/linker/CRT provenance, build metadata, link map, and link-input hashes. It must remain an `x86_64-linux-musl` static PIE with no interpreter or dynamic dependencies, using musl mallocng and no mimalloc. A runtime bump means new verified hashes, license/source review, rebuilt source provision, packaged validation, and the PLAN §8 exact-AppImage physical matrix. The Linux leg remains on Ubuntu 24.04 with a glibc 2.39 floor. Its private runtime now builds matched GStreamer 1.28.7 core/base/good/bad plus libnice 0.1.24 libraries and curated plugins before librice/WebKit: Noble's 1.24.2 accepts WebKit's configure checks but has incompatible transceiver, incoming-track and SDP behavior (#312), and the former 1.26 runtime is outside the maintained security branch. Keep those five source hashes, the pinned Meson source, license inventory, source-built scanner/PTP helpers, and source archives coherent. Only the PipeWire plugin remains distro-provided. The portal patch requests the legacy linear BGRA format understood by Noble's PipeWire 1.0.5 plugin. The output wrapper leaves Wayland's client library to the host EGL driver; it preserves the verified AppRun and output-tool binaries. Pre-merge and preview compiles use a hash-pinned `sccache`; never add a compiler cache — or any other restored build state — to the tagged release leg, which must reconstruct the runtime from its verified tuple. Those two compiling legs are `ci.yml`'s `Linux AppImage startup smoke` and `deploy.yml`'s Linux installer, and they share one cache entry per family (see the CI section's **Linux build caching**); bumping the tuple here edits `scripts/linux-webkit-runtime.env` or `scripts/patches/*.patch`, which is the *first* hash of both WebKit cache keys, so both legs correctly recompile once — widening `restore-keys` across that first hash to avoid the wait is precisely what the manifest-boundary split exists to forbid, because it would reuse a runtime built from a different tuple. The mutable Noble baseline is not bit-reproducible, the evidence is not legal sign-off, and distro security updates do not service the bundle.
...
- **Linux packaged runtime/startup** — Ubuntu 24.04, Rust 1.97.1, and
  ...
  runtime with matched GStreamer 1.28.7, libnice 0.1.24, and the x86_64 AppImage. CI validates the exact artifact's native
  libraries/subprocesses, sandbox + build-manifest/license inventory, ELF links,
  packaged-only GStreamer WebRTC/SCTP elements, codec/SRTP/GL pipelines, source-build identities, llama runtime, and a gnome-keyring Secret
...
- **Linux build caching** — **two** jobs compile WebKitGTK: `ci.yml`'s `Linux
  AppImage startup smoke` and `deploy.yml`'s Linux installer leg. Both install
  to `${RUNNER_TEMP}/studyvis-webkit-runtime`, and the `studyvis-webkitgtk-`,
  `studyvis-cargo-c-`, and `studyvis-sccache-webkit-` keys are **byte-identical
  across the two files on purpose** — one entry per family serves both jobs.
  Edit such a key in both workflows or in neither, and move a restore key and
  its save key together; a save key that drifts from its own restore key writes
  the entry where nothing looks for it. Nothing in CI checks this, so verify by
  hand: `grep -n 'studyvis-webkitgtk-\|studyvis-cargo-c-' .github/workflows/{ci,deploy}.yml`.
  `studyvis-linuxdeploy-` is the deliberate exception — `ci.yml` carries a `-ci`
  infix because the two jobs stage into different absolute paths and an archive
  of one cannot be unpacked over the other. **Key on what the validator keys
  on.** The WebKit key splits at the manifest boundary: first hash
  `linux-webkit-runtime.env` + `patches/*.patch` (what `expected_manifest` is
  derived from), second hash `build-linux-webkit-runtime.sh` +
  `generate-librice-third-party-notices.mjs` (which move the key but not the
  manifest), and `restore-keys` falls back across the second only — safe because
  `runtime_is_complete` byte-compares `BUILD-MANIFEST.txt`, re-checks the
  patch/notice SHAs and every packaged ELF, then `rm -rf`s and rebuilds on
  drift. The linuxdeploy and cargo-c entries take **no** `restore-keys` for the
  mirror-image reason: `prepare-linuxdeploy-tools.sh` runs
  `verify-linux-appimage-runtime.sh` over a reused tree and *aborts* rather than
  rebuilding, so a prefix fallback could only hand it a runtime it dies on. That
  same key is what makes editing `linux-appimage-runtime-sources.tsv` or
  `build-linux-appimage-runtime.sh` safe — both are byte-compared by `verify`,
  and hashing them into the key is what forces the clean rebuild. Restore/save
  is split rather than combined `actions/cache` because that action's post step
  is `post-if: success()`, so a packaging or Xvfb failure hours later discarded
  an already-validated multi-hour compile; save immediately after the step that
  validates, under the condition the entry records — `success()` for the three
  prefix caches, `!cancelled()` for sccache, because a *failed* compile still
  leaves object files worth keeping while a failed build must never be recorded
  as a complete runtime. Do not normalize those two conditions to each other.
  A cold compile is ~3.5h and a warm one is ~1s, so when a hit stops paying off
  suspect eviction from the 10 GB per-repository budget the rust-cache entries
  already crowd — the `du -sh` line in the build step exists to diagnose that
  from the log — before touching any key.
...
- **One-click:** the `Release prep` workflow (Actions tab, `.github/workflows/release-prep.yml`) bumps all five files, commits + atomically pushes `main` and a `vX.Y.Z` tag; that PAT-authenticated tag push triggers the release build. A cheap `Release request preflight` job runs first — shared version arithmetic (`scripts/next-version.sh`, the same script the bump itself uses), the tag still being free, a non-empty CHANGELOG section for the new version, and the PAT's presence — so a forgotten `## Unreleased` rename fails in seconds instead of after the ~20-minute quality gate. Its `auto_publish` input (**default on**) queues **Publish verified release** for the new tag as soon as the tag's release build exists; that dispatch waits on the shared `release-<tag>` concurrency group rather than racing the build, and queuing is not approving — every gate in `publish-release.yml` still runs and still fails closed. Untick it to keep the older dispatch-it-yourself flow. It needs the **`RELEASE_PAT`** secret — a fine-grained PAT scoped to this repo with Contents: read+write. `main` is protected by a ruleset requiring a PR plus the `All pre-merge checks` status check, and `GITHUB_TOKEN` cannot bypass that on a user-owned repo (Integration bypass actors are organization-only), so the bump commit is pushed with an owner PAT — the ruleset's one bypass actor is RepositoryRole 5 (admin). The gate job fails fast with that explanation if the secret is missing.
...
- **Publish:** unless `auto_publish` already queued it, run **Publish verified
  release** (`publish-release.yml`) with the tag alone once the draft verifier is green — it derives the AppImage
