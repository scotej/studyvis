...
- From V2-P6: add `topic_set`, `topic_change`, `break_request`, `break_approved`, `break_denied` to `AuditEventKind` + `AUDIT_KIND_LABELS` + `isAuditEventKind` in `src/lib/audit-types.ts` (V2-P6 added `ai_warning` / `ai_alert`); `tests/unit/audit.test.ts` currently uses `'break_request'` as the unknown-kind sentinel, so update it (replace with another out-of-scope kind, or split into a positive case for the new kinds) when the kind set grows.
- From V2-P6: `SessionView` already exposes `emitAudit` (local-append + broadcast) and `appendLocalAudit` (local-only) inside the hello/audit/pomodoro effect; route the V2-P7 break events through these helpers rather than building a parallel audit pipeline. `break_request` is local-only iff the AI verdict hasn't been resolved yet (mirror V2-P6's warning path); `break_approved` / `break_denied` should broadcast via `emitAudit` so peers see the outcome in real time.
- From V2-P6: the AI alert dispatcher's signed wire shape is the model for any new V2-P7 broadcast (e.g. a future `topic_change` data-channel message — ARCHITECTURE.md §7) — reuse the canonical-JSON serializer pattern in `aiAlerts.ts` so signatures verify byte-identically across peers.
...
- From V2-P6: `ai_warning` + `ai_alert` audit kinds are already produced + persisted via `audit_event_insert` (signed Ed25519 event whose `detail` carries `{ severity, reasoning }`). The report's per-event timeline + "top distractions" grouping should read `detail.reasoning` directly. `ai_warning` events appear ONLY on the off-task user's machine (never broadcast); the report consumer must accept "this peer has no `ai_warning` rows" without inferring anything about their focus.
- From V2-P6: the wire alert payload is signed but deliberately omits `deduction` / `scoreAfter` (V2-P5 carryover privacy invariant). When V2-P8 surfaces per-event deductions in the local-user's own report, those numbers must be reconstructed from `useFocusStore` (or a parallel `scoreLogStore` if V2-P8 adds one) — they are NOT in the audit DB.
- From V2-P7: `topic_set`, `topic_change`, `break_request`, `break_approved`, `break_denied` were added to `AuditEventKind` + `AUDIT_KIND_LABELS` + `isAuditEventKind` in V2-P7 — step 1 of V2-P8 ("Extend the AuditEvent kinds") is therefore PARTIAL: drop the additions but keep step 1's "wire them up in their producer code paths" framing — V2-P7 already produces `topic_change` (via the dialog) and the three `break_*` kinds (via `features/session/break.ts`); `topic_set` is reserved and still owes a producer (V2-P9's session-start input). The audit log row UI step still owes distinct icons + hover tooltips for the V2-P7 kinds (V2-P6 only covered `ai_warning` / `ai_alert`).
- From V2-P7: `break_request` is LOCAL-ONLY (mirrors the V2-P6 `ai_warning` privacy invariant). The report must NOT treat the absence of a peer's `break_request` row as evidence of anything — only `break_approved` / `break_denied` cross the wire.
- From V2-P7: the `topic_change` audit detail is `{ previous_topic, new_topic }`; the V2-P8 topic-timeline rendering should walk these in order to build a per-session topic history (start → topic_set or default 'Studying' → each topic_change → end).
...
- From V2-P5: V2-P7 replaced the hardcoded `topic: 'Studying'` argument with `getTopic: () => useSessionStore.getState().declaredStudyTopic`. V2-P9's required session-start topic input writes through `setDeclaredStudyTopic(value)` before the session flips to active; persist the same value to `sessions.declared_topic` on the new DB column.
...
- From V2-P6: `PEER_ALERT_TTL_MS` + `WARNING_TTL_MS` (both 30 s) are module-level constants in `src/features/ai/alertsUiStore.ts`. If V2-P9 ships a "tile-alert duration" slider in Settings → AI, surface them via the existing `__setAlertsUiRuntime` seam (or promote the TTL into store state). Today the values are fixed; the comment block documents the choice.
- From V2-P7: `useSessionStore.declaredStudyTopic` now exists (default 'Studying' via `DEFAULT_DECLARED_STUDY_TOPIC` from `@/stores/sessionStore`) and `setDeclaredStudyTopic` is the mutator. V2-P9 step 3's required session-start input should call `setDeclaredStudyTopic(value)` BEFORE flipping `useSessionStore.status` to active, then `emitAudit('topic_set', { topic: value })` from SessionView's hello/audit/pomodoro effect — `topic_set` is wired into the kind set + label map (V2-P7) but no producer exists yet.
- From V2-P7: `SampleLoopOptions.topic: string` was refactored to `getTopic: () => string` so a mid-session topic_change via the Ctrl+] dialog takes effect on the next inference tick. V2-P9's per-tick `sampleIntervalSec` slider should follow the same pattern (option (a) from the V2-P5 carryover) — replace the boot-time `state.sampleIntervalSec` assignment with a getter the scheduler calls each `tick()`. Both reads then share the same per-tick discipline.
- From V2-P7: the V2-P7 break-rule layer (`features/session/break.ts`) caps `MAX_BREAK_DURATION_SEC = 600` and `MAX_BREAKS_PER_SESSION = 4`. The "≤ 4 breaks per 2-hour session" reading from PLAN.md V2 was simplified to "4 total since session start"; for sessions that exceed 2 hours this over-restricts. If V2-P9 ships a session-duration cap or surfaces "long session" affordances, upgrade to a rolling 2-hour window in `evaluateBreakRules` using `useBreakStore.sessionStartedAt` (or a per-break timestamp log). Update `tests/unit/break-rules.test.ts` to cover the new boundary.
- From V2-P7: `useBreakStore` now tracks `lastBreakEndedAt` / `breaksThisSession` / `sessionStartedAt`; V2-P9's DB migration owes a `sessions.breaks_taken INTEGER NOT NULL DEFAULT 0` column if the report (V2-P8) surfaces "breaks taken" in the post-session report — otherwise the live store loses the value when SessionView resets at the next session start.
- From V2-P7: the floating Ctrl+] dialog window relies on `app.macOSPrivateApi: true` in `tauri.conf.json` and the `macos-private-api` Tauri feature flag (transparent windows on macOS need both). This is documented as App-Store-incompatible; V3's signing-creds workstream should re-confirm we're OK with that before re-enabling the updater plugin.
- From V2-P8: `sessions.declared_topic`, `sessions.score`, `sessions.focused_pct`, and `sessions.generated_at` already landed in `001_initial.sql` (amended in place — the file header authorizes pre-release amendments). V2-P9 step 2's migration is therefore PARTIAL on the sessions table; the models-table additions remain owed. If V2-P9 introduces `002_v2.sql` for the models table, do NOT redeclare the sessions columns there — keep them on `001_initial.sql`.
- From V2-P8: `sessions.declared_topic` is persisted from a NEW one-shot `useSessionStore.initialDeclaredTopic` field (set in `begin()`, never mutated). V2-P9 step 3's required session-start input must write the user's value into this field BEFORE the session flips to active — extend `begin()` with an `initialDeclaredTopic` arg or expose a setter callable while `status === 'idle'`. The mutable `declaredStudyTopic` field (driven by the Ctrl+] dialog's `topic_change` event) is still the live value the sample loop reads; do NOT collapse the two.
- From V2-P8: SessionView's V2-P5 reset effect now also calls `useAuditStore.getState().reset()` + `usePomodoroStore.getState().reset()` (handles the invite-while-on-report path where the user never clicks Close). V2-P9's session-start hooks must preserve this — don't relocate the resets to `begin()` or `buildLeaveHandler` without verifying the cross-session invariant survives.
- From V2-P8: the `focused_pct` value persisted in the sessions row comes from `useFocusStore.totalSamples` / `useFocusStore.onTaskSamples` counters (added in V2-P8). The V2-P5 carryover line about "V2-P9 owes `sessions.score`" is RESOLVED — both `score` and `focused_pct` are wired. V2-P9 only owes `topic_set` producer + the session-start input that writes `initialDeclaredTopic`.
- From V2-P8: `useAuditStore.flushPending()` was added so the post-session report query never races in-flight `audit_event_insert` calls. The leave handler in `lifecycle.ts` awaits it before `sessionsInsert` runs. If V2-P9 changes the session-end ordering, keep the flush ahead of the sessions upsert so re-opened reports stay byte-identical to the first render.
- From V2-P8: the V2-P3 `SessionEndedSplash` component + story have been retired (the splash was always documented as a placeholder for this report). `useSessionStore.endedSnapshot` + the `peerNames`/`durationSeconds` plumbing in `buildLeaveHandler` were dropped along with them. If V2-P9 needs a transient "session ended" affordance separate from the report, build it fresh — there is no existing surface to extend.
