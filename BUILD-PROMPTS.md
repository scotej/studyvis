...
- D6 — All three UX tunables were deferred by V3-P8 as record-only: (a) `alertsUiStore` `PEER_ALERT_TTL_MS` / `WARNING_TTL_MS` as a user-facing tile-alert-duration control — defer, because surfacing a new settings control is a new feature and the existing 30s/5s constants haven't been reported as wrong; (b) `evaluateBreakRules` rolling 2-hour window instead of "4 breaks per session" — defer, because changing the deny condition is a behaviour change with no in-the-wild evidence the current rule frustrates the friends-only audience; (c) `sessions.breaks_taken` column on the post-session report — defer, because it's a schema change + new report rendering, and the existing audit-log row for each break already shows the count in the timeline. Record all three in "What's intentionally not in 1.0" with these reasons.
...
- From V3-P8: the boot-paint extraction (three near-identical inline pre-paint scripts in `index.html` + `ai-dialog.html` for theme, windowStyle, reduceMotion) was considered but DEFERRED to avoid a first-paint regression — moving the logic to a `/public/boot-paint.js` would add a fetch round-trip to the critical pre-paint path even when synchronous. The inline-script approach is the intentional choice; record as a known-but-deliberate duplication in "What's intentionally not in 1.0" with that reason.
- From V3-P8: `LEGACY_THEME_LOCALSTORAGE_KEY` in `src/stores/settingsStore.ts` keeps its V1-P11 spelling but now doubles as the boot-paint cache for theme + windowStyle + reduceMotion. The V3-P7 carryover suggested introducing `THEME_LOCALSTORAGE_KEY` as a forward-looking alias for new code. V3-P8 left both names in place; record as a post-1.0 ergonomic rename — the constant is correctly scoped today, just historically named.
- From V3-P8: the centralised strings module (`src/strings.ts`) and the `scripts/check-strings.ts` guard cover toast + notification surfaces — every site V3-P8 found drifting. JSX text and `aria-label` inline strings are mostly hoisted but not exhaustively guarded; the guard is intentionally targeted (toast / notification are the actual drift cases observed across phases). V3-P9 should note this scope explicitly in the release report — "guard is targeted, not exhaustive; manual review remains the second pair of eyes".
- From V3-P8: `/style` route at `src/routes/StyleGuide.tsx` covers `src/components/ui/` primitives reasonably well but is missing roughly half the app-specific composed components (VideoTile, AuditLogPanel, ScoreGauge, etc.). DESIGN-SYSTEM §7 rule 5 reads "every primitive and every status state side by side" — V3-P9 should either expand the route to cover the composed layer, or update §7 rule 5 to scope it to `ui/` primitives only (Storybook already covers the composed layer through its stories). Pick one; don't leave the promise/reality drift.
