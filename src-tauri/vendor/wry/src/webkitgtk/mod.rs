...
use webkit2gtk::{
  ...
  PrintOperationExt, Settings, SettingsExt, URIRequest, URIRequestExt, UserContentInjectedFrames,
...
impl InnerWebView {
  ...
  fn create_webview(
    ...
    // StudyVis Linux patch: RTCPeerConnection has an independent compile-time
    // WebKit gate and a runtime PeerConnectionEnabled preference. The bundled
    // WebKitGTK enables the former; supplying these settings to WebView::builder
    // guarantees the latter is present in the initial PageConfiguration.
