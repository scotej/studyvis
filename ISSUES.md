...
**Status.** **fixed** — the second of the three options: the opacity check is its own invariant (`indicator.opacity_disagrees_with_store`) and dwells on *its own samples* rather than on watchdog ticks. `Invariant` gained an optional `samplesOn`, and a tick where it returns false neither counts toward the dwell nor clears it, so `dwellTicks: 2` now means two consecutive style reads — about 10 s apart under `PTT_WATCHDOG_STYLE_EVERY` — which is the right shape anyway, since a stuck-lit badge is a steady state rather than a transition. `samplesOn` is deliberately checked *after* applicability, so leaving the session still clears an open violation instead of stranding it until the next sample. The style cadence is untouched: no extra recalculation per second.

The suite is the other half of the story, because it is why this survived: the existing badge test fed a style reading on **every** tick, so it proved the predicate and never its reachability. It now asserts the new id, and three cases sit beside it — the stuck badge driven at the real 1-in-10 cadence (which fails without `samplesOn`, checked by reverting the guard), an agreeing badge staying silent across the same cadence, and the session-exit clear. `indicator.disagrees_with_store` keeps the `selfLit` clause it always ran on and is unchanged in behaviour.
...
**Status.** **fixed** — the discriminator no longer uses a clock. `PttListener` holds a boolean across the `press()`/`release()` call in a `try`/`finally` and the store subscriber reads that instead of comparing timestamps, which is exact rather than heuristic: the subscriber runs synchronously *inside* the mutation, so the latch is true for precisely the changes a native edge is already recording and false for every other one, however long the main thread stalls. That synchronous-notification contract is what the latch rests on, so `tests/unit/ptt-store.test.ts` now pins it directly — `press()` and `release()` each assert the subscriber ran before the call returned. Without it a future zustand change would reintroduce the phantom `reset` silently, which is how this was found in the first place.
...
**Status.** **partly fixed; the capture behaviour is open.** The diagnostic half is closed: every extra-display outcome now records `display.acquire_stopped` keyed on the underlying DOMException name rather than the mapped code, at `warn` when the webview refused the call and `info` when the user dismissed the picker, with the target and achieved display counts beside it. The capture half is deliberately not attempted here — a second synchronous `getDisplayMedia()` inside the same click cannot work either, because the Screen Capture spec's algorithm both requires transient activation ("If the relevant global object of this does not have transient activation, return a promise rejected with a DOMException object whose name attribute has the value `InvalidStateError`") and then performs "Consume user activation with this's relevant global object" before prompting, so the first call in a gesture spends the only activation there is. Honouring the setting therefore needs a design decision (a native multi-surface capture path, or withdrawing the option and saying so) rather than a fix. Nothing regresses meanwhile: the primary stream is unaffected, the session continues, and a machine that *does* grant a second surface still composites it.

### I95 — Sev2

`src/features/session/useWrittenTimeline.ts`

**Evidence.** The #236 write-up effect depends on `timeline`, and a finished pass hands a brand-new record back through it — `onWritten` → `Report` stores it → the prop is a fresh object → the effect re-runs. Its cleanup releases the once-only latch on every such run (that release exists so React 19 Strict Mode, which tears the first effect down mid-probe, cannot strand the latch and stop the write-up happening in dev at all), and the completion guard covered only the first attempt: `if (timeline && attempt === 0) return`. So a rewrite re-triggered itself on its own output and never stopped — a full local-model pass over the whole session (up to five 120-second chunk requests), a sidecar start/stop, and a `session_timelines` upsert, on repeat for as long as the report stayed open. Reachable from the button rather than a corner: "Write it up again" renders exactly when `source` is `'mixed'` or `'observations'`, which is what a session gets whenever the model could not narrate every window — an unreachable sidecar being the common way there — and from Retry-then-success on the failed state.

**Status.** **fixed** — `createWriteUpLatch` replaces the two conventions maintained across three call sites with one object holding both: an in-flight `claim` the cleanup still `release`s (Strict Mode unchanged) and a `complete` that is permanent for its key and is set before `onWritten` fires, so the re-run that record triggers reads the key as done. Unit-tested as a sequence rather than as a claim about React scheduling — the exact replay that used to loop is a test.

### I96 — Sev2

`src/features/session/sessionTimeline.ts`

**Evidence.** `ensureSidecar` starts llama-server and only then awaits `waitForSidecarReady`, but the caller learns it owns that engine from the function's RETURN value. A readiness failure throws instead — `SIDECAR_HEALTH_TIMEOUT_MS` is 90 s, which a cold CPU start of a multi-GB vision model plus projector can outrun, and an errored child rejects immediately — so `generateSessionTimeline`'s `sidecar` local stayed null, its `finally`'s `sidecar?.startedHere` guard skipped the stop, and the engine stayed resident with the model loaded and no consumer for the rest of the process. The report itself was unaffected (an unreachable engine already falls back to the deterministic digest and labels it), so nothing surfaced. The module's own header says ownership follows `benchmark.ts`, whose `finally` is explicit about exactly this case: stop "even on startSidecar / waitForHealthy / chat-completion failures, so a partially-initialised sidecar can't leak past benchmark exit". A resident engine also makes the updater's install worse — the reason `sampleLoop`'s `stop()` logs a failed sidecar stop at all.

**Status.** **fixed** — released where ownership is known, inside `ensureSidecar` on the way out, sharing one `releaseSidecar` with the success path so the session-active guard cannot drift between them. Regression-tested both ways: an engine this pass started is stopped, one that was already running is not.

### I97 — Sev3

`src/features/updater/updaterStore.ts`

**Evidence.** `check()` returns a plugin `Update`, which extends `Resource` — a handle on a Rust-side resource, and after a download on the staged bytes too. `checkNow` has four exits once it holds one and only two treated it as owned: `ready` keeps it as `pending` for `install()`, and the I71 blocked path closes it with a comment saying why. A failed download (a dropped connection on the six-hourly background check) and a session starting during the check both returned without closing, while the store set `pending: null` and called that "nothing staged". Neither freed until process exit.
...
**Status.** **fixed** (2026-08-28) — `unsound = "all"` is now set explicitly beside the deliberate `unmaintained = "workspace"`, with a comment on why the two get different scoping: an unmaintained transitive crate is a fact about someone's release cadence, while an unsound one can crash the binary regardless of where it sits.

Turning it on immediately proved the point — the gate caught **two further unsound advisories nobody knew were in the shipped graph**, both trivially fixable:

- **RUSTSEC-2026-0190**, `anyhow 1.0.102` (beneath `tauri`): `Error::context` followed by `Error::downcast_mut` builds a `&mut` from a borrow chain containing a shared reference, which Miri rejects as UB. Fixed by `cargo update -p anyhow --precise 1.0.103`.
- **RUSTSEC-2026-0221**, `event-listener 5.4.1` (via `notify-rust`/`zbus`/`async-broadcast`, i.e. the notification plugin): `StackSlot` lets `!Send` tags cross thread boundaries. Fixed by `cargo update -p event-listener --precise 5.4.2`.

Both are semver-compatible patch bumps, lockfile-only, and `THIRD-PARTY-NOTICES.{txt,json}` plus the `src-tauri/resources/` copies were regenerated for them (same licenses, same license-file hashes — only the version, download URL and source checksum move).
