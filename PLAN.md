...
- **Linux support** — *trigger: WebKitGTK `getDisplayMedia` re-verified on a current distro.* Linux has been gated on one unanswered question since V0; the unblock is concrete, not open-ended:
  1. Re-run the V0 smoke test under current WebKitGTK — `getUserMedia` + `getDisplayMedia` + a trystero rendezvous between two machines. **Half-answered (2026-07):** on WebKitGTK 2.52.3 / Ubuntu 24.04 aarch64, `getUserMedia` opens camera *and* microphone against virtual devices. `getDisplayMedia` is exposed as a function but has **not** been verified to succeed — it wants a desktop portal — so the trigger has not fired.
  2. The keychain half is **done ahead of the trigger**: `keyring` carries a `sync-secret-service` backend under `cfg(target_os = "linux")`, so the identity, friends-backup and HF-token commands compile and run there, and `ci.yml` has a Linux leg keeping that honest. This does **not** make Linux a release target — it makes the app runnable on a Linux dev box, which is what a maintainer needs to answer step 1 properly. Still outstanding for a release: an `.AppImage` job in `release.yml`, and confirming the battery fallback (`system_battery` already returns a safe `on_battery: false` default when UPower is absent).
  3. If `getDisplayMedia` still fails: ship **AI-off Linux** rather than blocking the whole platform — body-doubling needs only camera + mic, so the core study experience is fully available. Since #96 that build also loses screen sharing (it is the second `getDisplayMedia` caller, alongside the AI loop); receiving a friend's shared screen is unaffected, since that arrives as an ordinary WebRTC stream.
