...
- AI enablement guard — AI remains off until the user explicitly enables it. If the selected model has no current benchmark, show a warning before enabling; the user may continue with the 5s cadence and conservative 300s request-timeout fallbacks or benchmark first. Without a p95 baseline, the *duration* half of slowdown backoff is disabled — there is nothing to call a check slow against — but the *starvation* half still applies: a check that leaves the app itself unscheduled for seconds stretches the cadence on the first occurrence, benchmark or not (#269).
