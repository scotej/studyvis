...
- **Network footprint**: a handful of long-lived WebSockets to a small curated set of public Nostr relays while idle (a few KB/hour; the inbox + presence rooms pin the full relay list and stay open — see ARCHITECTURE §4). During sessions: full-mesh WebRTC (peer-to-peer) for audio/video. Approximately 15% of network configurations require a TURN relay to connect; no public TURN ships today (the old free public endpoints are dead — see §7 and ARCHITECTURE §4), so those sessions can fail until the user adds their own TURN server in Settings → Network.
