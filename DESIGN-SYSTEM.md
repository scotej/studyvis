...
- Visible focus ring (`shadow.glow` + `border.strong`) on focused element. Implementation uses the existing tokens via per-component `focus-visible:` utilities; the global `:focus-visible { outline: none }` reset relies on every interactive element shipping its own ring (enforced by the V3-P7 axe gate, which fails on missing focus indicators).
