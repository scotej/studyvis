...
| `ScreenShareViewer` | Native fullscreen view of one shared screen, opened from a screen tile's expand control. The modal fills the webview as a browser fallback; Escape or leaving native fullscreen closes it rather than arming leave-the-session. |
...
- **Post-session report timeline**: 360 high by default, resizable from 240 to 640 (`sizes.reportTimeline*`). Overflow stays inside the themed `ScrollArea`; its narrow scrollbar appears only while the timeline is hovered or actively scrolled, so long reports remain readable without adding a permanent visual rail.
