...
- **Video grid**: flex-wrap, filling everything the session view has between the media banner and the footer. Tiles are always 16:9, and are sized to the largest they can be: the grid measures its slot and picks the row/column split that maximizes tile size for the current tile count, so two people in a window shorter than 32:9 of usable area stack rather than sit side by side (#95). Floor of 180 px tall (`sizes.videoTileMinHeight`) — reachable only with several screen shares open at the window minimum, and the grid scrolls internally rather than shrink past it. No ceiling.
